...
public class WinstreakBoard extends LeaderboardBase {
	...
	private final Map<UUID, Integer> lifetimeWinstreak = new HashMap<>();
	...
	private static final AtomicInteger ENTITY_ID = new AtomicInteger(500000); // custom id base, offset from other boards

	// Board title location (Winstreak)
	private static final Location WINSTREAK_TITLE_LOC = new Location(null, 209.5, 107.5, 704.4);
	...
	private String sqlForScope(LeaderboardScope scope) {
		final String col = "BestWinstreak";
		final String metric = "BestWinstreak";
		final String label = "WinstreakVal";
	...
	public void asyncUpdate() throws SQLException {
			...
			for (LeaderboardScope scope : LeaderboardScope.values()) {
				...
				try (ResultSet rs = st.executeQuery(sqlForScope(scope))) {
					while (rs.next()) {
						...
						int value = rs.getInt("WinstreakVal");
		...
		lifetimeWinstreak.putAll(topValues.getOrDefault(LeaderboardScope.LIFETIME, Collections.emptyMap()));
	}
	...
	public void updateLeaderboard(boolean init) {
		...
		Location title = ensureWorld(WINSTREAK_TITLE_LOC, main.getLobbyWorld());
		sendArmorStandPacketGlobalSelective(title, ChatColor.YELLOW + "" + ChatColor.BOLD + ChatColor.UNDERLINE + "Best Winstreak");
		...
		for (int i = 0; i < lifetimeTopIds.size() && rank <= 10; i++, rank++) {
			...
			Integer v = lifetimeWinstreak.getOrDefault(id, 0);
	...
	public void showToViewer(Player viewer, LeaderboardScope scope) {
		...
		int yourVal = getScopedWinstreakFor(viewer, scope);
	...
	private int getScopedWinstreakFor(Player viewer, LeaderboardScope scope) {
		try {
			if (scope == LeaderboardScope.LIFETIME) {
				PlayerData d = main.getDataManager().getPlayerData(viewer);
				return (d != null) ? d.bestWinstreak : 0;
			}
			int current = 0;
			PlayerData d = main.getDataManager().getPlayerData(viewer);
			if (d != null) current = d.bestWinstreak;
			else {
				try (PreparedStatement ps = main.getDatabaseManager().getConnection()
						.prepareStatement("SELECT BestWinstreak FROM PlayerData WHERE UUID=? LIMIT 1")) {
			...
			try (PreparedStatement ps = main.getDatabaseManager().getConnection().prepareStatement(
					"SELECT total_value FROM scb_stat_snapshots WHERE uuid=? AND metric='BestWinstreak' AND period=? AND period_start=? LIMIT 1")) {
				ps.setString(1, viewer.getUniqueId().toString());
