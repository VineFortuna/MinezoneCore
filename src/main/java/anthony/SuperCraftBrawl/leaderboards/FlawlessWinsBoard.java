...
public class FlawlessWinsBoard extends LeaderboardBase {
	...
	private final Map<UUID, Integer> lifetimeFlawlessWins = new HashMap<>();
	...
	private static final AtomicInteger ENTITY_ID = new AtomicInteger(400000); // custom id base, offset from KillsBoard

	// Board title location (FlawlessWins)
	private static final Location FLAWLESS_TITLE_LOC = new Location(null, 169.5, 107.5, 704.5);
	...
	public void asyncUpdate() throws SQLException {
			...
			for (LeaderboardScope scope : LeaderboardScope.values()) {
				...
				try (ResultSet rs = st.executeQuery(sqlForScope(scope))) {
					while (rs.next()) {
						...
						int value = rs.getInt("FlawlessWinsVal");
		...
		lifetimeFlawlessWins.putAll(topValues.getOrDefault(LeaderboardScope.LIFETIME, Collections.emptyMap()));
	}
	...
	public void updateLeaderboard(boolean init) {
		...
		Location title = ensureWorld(FLAWLESS_TITLE_LOC, main.getLobbyWorld());
		sendArmorStandPacketGlobalSelective(title, ChatColor.YELLOW + "" + ChatColor.BOLD + ChatColor.UNDERLINE + "Lifetime Flawless Wins");
		...
		for (int i = 0; i < lifetimeTopIds.size() && rank <= 10; i++, rank++) {
			...
			Integer v = lifetimeFlawlessWins.getOrDefault(id, 0);
	...
	public void showToViewer(Player viewer, LeaderboardScope scope) {
		...
		int yourVal = getScopedFlawlessWinsFor(viewer, scope);
	...
	private int getScopedFlawlessWinsFor(Player viewer, LeaderboardScope scope) {
		try {
			if (scope == LeaderboardScope.LIFETIME) {
				PlayerData d = main.getDataManager().getPlayerData(viewer);
				return (d != null) ? d.flawlessWins : 0;
			}
			int current = 0;
			PlayerData d = main.getDataManager().getPlayerData(viewer);
			if (d != null) current = d.flawlessWins;
			else {
			...
			try (PreparedStatement ps = main.getDatabaseManager().getConnection().prepareStatement(
					"SELECT total_value FROM scb_stat_snapshots WHERE uuid=? AND metric='FlawlessWins' AND period=? AND period_start=? LIMIT 1")) {
				ps.setString(1, viewer.getUniqueId().toString());
