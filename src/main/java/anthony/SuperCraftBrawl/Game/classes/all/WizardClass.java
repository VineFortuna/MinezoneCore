...
public class WizardClass extends BaseClass {
    ...
    private static final int TELEPORT_COOLDOWN_MS = 15 * 1000;
    ...
    private ItemStack makePlainWand() {
        ItemStack wand = ItemHelper.setDetails(new ItemStack(Material.STICK),
                instance.color("&rMagic Wand &7(Right Click)"));
        return wand;
    }

    private ItemStack makeWand(int sharpness, int knockback) {
        ItemStack wand = ItemHelper.setDetails(new ItemStack(Material.STICK),
                instance.color("&rMagic Wand"));
    ...
    private void pickSpell() {
        ...
        switch (spell) {
            case 1:
                ...
                weapon = makeWand(4, 1);
                instance.TellAll(instance.color("&6&lWizard> &rA rift opens before me... &c&lTeleport&r!"));
                break;
            case 2:
     ...
     */
    /**
     * Played at the destination — lighter arrival particles so it is not too spammy.
     */
    private void playAppearEffect(Location loc) {
        World world = loc.getWorld();

        // Lighter contracting rings
        int rings = 3;
        int points = 8;
        ...
        // Small center pop so the arrival still feels noticeable
        for (int i = 0; i < 3; i++) {
            world.playEffect(loc.clone().add(0, 1, 0), Effect.PORTAL, 1);
        }
    ...
    private void useFireball() {
        if (wizard.getTime() < FIREBALL_COOLDOWN_MS) {
            ...
            player.sendMessage(instance.color("&c&l(!) &rBlaze Powder is still on cooldown for &a" + seconds + "s"));
    ...
    private void keepProjectileFlyingStraight(final ItemProjectile proj, final Vector velocity) {
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                // Failsafe so it doesn't run forever if something weird happens
                if (ticksAlive > 100) {
                ...
                // Re-apply the same velocity every tick to cancel gravity
