...
public class JebClass extends BaseClass {
    ...
    private void applyPushEffect(Player targetPlayer, Vector direction, double distance) {
        // Set last damage cause to this player so kill credit is correctly attributed
        // if the target dies from fall/void after being knocked back
        EntityDamageByEntityEvent lastDmg = new EntityDamageByEntityEvent(
                player, targetPlayer,
                EntityDamageByEntityEvent.DamageCause.ENTITY_ATTACK, 0.0
        );
        targetPlayer.setLastDamageCause(lastDmg);
