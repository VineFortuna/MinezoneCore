...
public class HalloweenHuntManager implements Listener {
	...
	private static final List<int[]> BASKETS = Arrays.asList(new int[] { 197, 105, 689 }, // map selection (#1)
	...
	public void onInteract(PlayerInteractEvent e) {
		...
		p.sendMessage(core.color("&8[&6&lHUNT&8] &rYou found &cBasket #" + humanNumber + "&a! &7(&e" + foundTotal
				+ "&7/&e" + TOTAL + "&7)"));
	...
	private void grantMilestoneReward(Player p, int foundTotal) {
		switch (foundTotal) {
		case 1:
			p.sendMessage(core.color("&8[&6&lHUNT&8] &rYou were rewarded with &e25 Tokens"));
			giveTokensReward(p, 25);
			break;
		case 2:
			p.sendMessage(core.color("&8[&6&lHUNT&8] &rYou unlocked: &6&lTrick-or-Treater &rtitle cosmetic"));
			break;
		case 3:
			p.sendMessage(core.color("&8[&6&lHUNT&8] &rYou unlocked: &6Ritual &rwin effect"));
			break;
		case 4:
			p.sendMessage(core.color("&8[&6&lHUNT&8] &rYou unlocked: &9&lCandy Aura &rcosmetic"));
			break;
		case 5:
			p.sendMessage(core.color("&8[&6&lHUNT&8] &rYou were rewarded with &e500 EXP"));
			giveExpReward(p, 500);
			break;
		case 6:
			p.sendMessage(core.color("&8[&6&lHUNT&8] &rYou unlocked: &6&lFreddy &routfit cosmetic"));
			break;
		case 7:
			p.sendMessage(core.color("&8[&6&lHUNT&8] &rYou unlocked: &6Pumpkin Pie &rdeath particles"));
			break;
		case 8:
			p.sendMessage(core.color("&8[&6&lHUNT&8] &rYou unlocked: &6&lFreddy Fazbear &rtitle cosmetic"));
			break;
		case 9:
			p.sendMessage(core.color("&8[&6&lHUNT&8] &rYou were rewarded with &e30 Tokens &rand &e700 EXP"));
	...
	private void reward(Player p) {
		Bukkit.broadcastMessage(core
				.color("&8[&6&lHALLOWEEN HUNT&8] &e" + p.getName() + " &6has found all 10 Baskets! Happy Halloween!"));
		...
		// Nice mixed palette
		org.bukkit.Color[] palette = new org.bukkit.Color[] { org.bukkit.Color.fromRGB(255, 165, 0), // orange
				org.bukkit.Color.fromRGB(128, 0, 128), // purple
				org.bukkit.Color.fromRGB(0, 0, 0), // black
				org.bukkit.Color.fromRGB(255, 255, 255), // white
				org.bukkit.Color.fromRGB(0, 255, 255), // cyan
				org.bukkit.Color.fromRGB(50, 205, 50), // lime
				org.bukkit.Color.fromRGB(255, 0, 0), // red
				org.bukkit.Color.fromRGB(255, 255, 0), // yellow
				org.bukkit.Color.fromRGB(0, 0, 255), // blue
				org.bukkit.Color.fromRGB(255, 0, 255) // magenta
		};

		FireworkEffect.Type[] shapes = new FireworkEffect.Type[] { FireworkEffect.Type.BALL,
				FireworkEffect.Type.BALL_LARGE, FireworkEffect.Type.STAR, FireworkEffect.Type.BURST,
				FireworkEffect.Type.CREEPER };

		for (int i = 0; i < 10; i++) {
			// small random offset around player
			Location spawn = base.clone().add((rand.nextDouble() - 0.5) * 2.5, 0, (rand.nextDouble() - 0.5) * 2.5);

			Firework fw = (Firework) w.spawnEntity(spawn, EntityType.FIREWORK);
			FireworkMeta meta = fw.getFireworkMeta();

			// random power (height)
			meta.setPower(1 + rand.nextInt(2)); // 1 or 2

			// pick 2-3 random colors for color + fade
			org.bukkit.Color c1 = palette[rand.nextInt(palette.length)];
			org.bukkit.Color c2 = palette[rand.nextInt(palette.length)];
			org.bukkit.Color c3 = palette[rand.nextInt(palette.length)];

			FireworkEffect.Builder effect = FireworkEffect.builder().with(shapes[rand.nextInt(shapes.length)])
					.withColor(c1, c2).withFade(c3).flicker(rand.nextBoolean()).trail(true);
