...
public class GenericModeGUI implements InventoryProvider {
    ...
    /**
     *
     * Creates a Comparator to sort games by wanted order.
     * Order:
     * 1. Waiting Games (Lobby with at least a player on it)
     *     a. Number of players
     *     b. Map name
     * 2. Started Games (Spectate)
     *     a. Map Name
     * 3. Lobby Games (Lobby with 0 players on it)
     *     a. Map Name
     *
     */
    @Override
...
//            System.out.println("Processing map: " + mapName + ", line: " + line + ", column: " + column);
            ...
            if (gameInstance != null) {
                ...
                if (gameInstance.state == GameState.WAITING) {
                    ...
                    contents.set(rows, column, ClickableItem.of(ItemHelper.setDetails(new ItemStack(Material.EYE_OF_ENDER),
                            "&e&l" + mapName,
                            "&a" + state,
                            "&rPlayers: &e" + gameInstance.players.size() + "/" + gameInstance.gameType.getMaxPlayers(),
                            "&rSpectators: &e" + gameInstance.spectators.size(),
                            "&rGame Time: &e" + gameInstance.gameTime + "m",
                            "",
