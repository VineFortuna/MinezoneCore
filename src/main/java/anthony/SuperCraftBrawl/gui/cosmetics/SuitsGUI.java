...
public class SuitsGUI implements InventoryProvider {
	...
	public void init(Player player, InventoryContents contents) {
		...
		int col = 1;
		for (Cosmetic outfit : main.getCosmeticManager().registry().getAll(CosmeticCategory.OUTFIT)) {
			contents.set(1, col, CosmeticGuiUtil.toggleItem(main, main.getCosmeticManager(), outfit, player,
					outfit.getIcon(player), inv));
			col++;
		}
