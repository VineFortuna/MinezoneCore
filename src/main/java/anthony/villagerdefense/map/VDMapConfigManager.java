...
/**
 * Loads/saves the single VillagerDefense map's configuration to a YAML
 ...
 */
public class VDMapConfigManager {

	private final Core main;
	private final File file;
	private VDMapConfig config;

	public VDMapConfigManager(Core main) {
		this.main = main;
		this.file = new File(new File(main.getDataFolder(), "villagerdefense"),
				VDGameConstants.MAP_WORLD_NAME + ".yml");
		load();
	}

	public VDMapConfig getConfig() {
		return config;
	}

	public void load() {
		if (!file.exists()) {
			config = new VDMapConfig(VDGameConstants.MAP_WORLD_NAME);
			return;
		}

		YamlConfiguration yaml = YamlConfiguration.loadConfiguration(file);
		VDMapConfig loaded = new VDMapConfig(VDGameConstants.MAP_WORLD_NAME);
		loaded.setLobby(readLocation(yaml, "lobby"));

		for (int i = 1; i <= VDMapConfig.TEAM_COUNT; i++) {
			String base = "teams." + i + ".";
			VDTeamSite site = loaded.getTeamSite(i);
			site.setSpawn(readLocation(yaml, base + "spawn"));
			site.setVillagerLoc(readLocation(yaml, base + "villager"));
	...
	public void save() {
		...
		for (VDTeamSite site : config.getTeamSites().values()) {
			String base = "teams." + site.getId() + ".";
			writeLocation(yaml, base + "spawn", site.getSpawn());
			writeLocation(yaml, base + "villager", site.getVillagerLoc());
		...
		try {
			...
			main.getLogger().warning("Failed to save VillagerDefense map config: " + e.getMessage());
		}
	}
