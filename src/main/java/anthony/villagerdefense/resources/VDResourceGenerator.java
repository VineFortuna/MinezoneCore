 ...
 * see VDEconomy. Uses plain dropItem (not dropItemNaturally) with velocity
 * zeroed so the item sits exactly where it's generated instead of scattering.
 * Owns a VDGeneratorDisplay for the cosmetic spinning block + hologram above
...
public class VDResourceGenerator {
	...
	private final VDGeneratorDisplay generatorDisplay;
	private BukkitTask task;
	private int elapsedSeconds = 0;

	public VDResourceGenerator(VDGameInstance instance) {
		this.instance = instance;
		this.generatorDisplay = new VDGeneratorDisplay(instance);
	}
	...
	private void tick() {
		...
		generatorDisplay.updateCountdown(VDResourceType.EMERALD,
				EMERALD_INTERVAL_SECONDS - (elapsedSeconds % EMERALD_INTERVAL_SECONDS));
		generatorDisplay.updateCountdown(VDResourceType.DIAMOND,
