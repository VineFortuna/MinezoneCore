...
/**
 * Full configuration for one FlagDefense map: the lobby point, each
 * team's site, and the shared emerald/diamond generator points. Filled in
 * via /fdsetup and persisted by FDMapConfigManager.
 */
public class FDMapConfig {
	...
	private final Map<Integer, FDTeamSite> teamSites = new LinkedHashMap<>();
	...
	public FDMapConfig(String worldName) {
		this.worldName = worldName;
		for (int i = 1; i <= TEAM_COUNT; i++) {
			teamSites.put(i, new FDTeamSite(i));
		}
	}
	...
	public FDTeamSite getTeamSite(int id) {
		return teamSites.get(id);
	}

	public Map<Integer, FDTeamSite> getTeamSites() {
		return teamSites;
	}
	...
	/** Returns missing pieces of config, for /fdsetup list. Empty if fully configured. */
	public java.util.List<String> findMissing() {
		java.util.List<String> missing = new java.util.ArrayList<>();
		if (lobby == null) missing.add("lobby");
		for (FDTeamSite site : teamSites.values()) {
			String color = FDGameConstants.TEAM_NAMES[site.getId() - 1];
